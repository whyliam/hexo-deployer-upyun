    ...
    try {
      ...
      let getRemoteList = (() => {
        var _ref2 = _asyncToGenerator(function* () {
          let data = yield client.getFile('.file_list.json', null);
          if (data != undefined) {
            return data;
          } else if (data == false) {
            return [];
      ...
      let processRemove = (() => {
        var _ref4 = _asyncToGenerator(function* (removeList) {
          for (let file of removeList) {
            let data = yield client.deleteFile(file);
      ...
      let processRemoveDir = (() => {
        var _ref5 = _asyncToGenerator(function* (removeDirList) {
          ...
          for (let dir of removeDirList) {
            ...
            while (try_times--) {
              let data = yield client.deleteFile(dir);
      ...
      let processMkdir = (() => {
        var _ref6 = _asyncToGenerator(function* (mkdirList) {
          for (let dir of mkdirList) {
            let data = yield client.makeDir(dir);
      ...
      let processPut = (() => {
        var _ref7 = _asyncToGenerator(function* (putList) {
          for (let file of putList) {
            ...
            let data = yield client.putFile(file, fileContent);
      ...
      let putFileList = (() => {
        var _ref8 = _asyncToGenerator(function* (fileList) {
          let data = yield client.putFile('.file_list.json', Buffer.from(JSON.stringify(fileList)));
      ...
      let localList = yield getLocalList(public_dir); // Get diff list

      function getDiffList(remoteList, localList) {
        let removeList = [],
            putList = [],
            removeDirList = [],
            mkdirList = []; // Determine which files to remote and put
        ...
        for (let remote of remoteDirs) {
          ...
          if (local) {
            localDirs.splice(index, 1); // Get diff of files and dirs in the dir
